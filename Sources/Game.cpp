...
// TODO: PlayBGM()関数を使って、BGMを再生する。(G) HW16A221 吉田和成
// TODO: PlaySE()関数を使って、弾の発射時とターゲットに当たった時にSEを再生する。(H)
>>>>>>> 70b5849e7e3da6161c4f6aede11dceb8546b4fb1
...
void Update()
{
    ...
    //雲の移動
    if (cloudPos.x < 310) {
        cloudPos.x += 30 * Time::deltaTime;
    }else{
        cloudPos.x =-540;
    }
    
    //砲台の移動
    if(cannonPos.y < -65&&(cannonMove==0||cannonMove==2)){
        cannonMove=0;
    }else if(cannonPos.y>-85&&(cannonMove==0||cannonMove==2)){
       cannonMove=1;
    }else if(cannonPos.y<-145){
        cannonMove=2;
    }
    if (cannonMove==0||cannonMove==2){
